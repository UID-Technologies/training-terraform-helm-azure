...
resource "azurerm_linux_virtual_machine" "vm" {
  ...
  # cloud-init script to install nginx and create index.html
  custom_data = base64encode(<<-EOF
                #!/bin/bash
                apt-get update -y
                apt-get install -y nginx
                echo "<h1>Welcome to Terraform Demo</h1>" > /var/www/html/index.html
...
# az vm show -d -g rg-tf-vnet -n vm-tf-lab --query publicIps -o tsv